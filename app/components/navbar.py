...
def clientDropdown(client):
    try:
        ...
        drop = dcc.Dropdown(
            id="client-dropdown", options=options, value=1, clearable=False
        )
        return drop
    except:
        return dcc.Dropdown(
...
def initialDrop():
    options = [{"label": "Volkno Titles", "value": 1}]
    return dcc.Dropdown(id="client-dropdown", options=options, value=1, clearable=False)
...
def staticNavbar():
    nav = dbc.Navbar(
        dbc.Container(
            [
                dbc.Col(dbc.NavbarBrand("Choose Client ", href="#"), sm=3, md=2),
                dbc.Col(initialDrop()),
            ]
        ),
        color="white",
    ...
    component = dbc.Navbar(
        children=[
            html.A(
                ...
                dbc.Row(
                    dbc.Col(
                        dbc.NavbarBrand(f"{title} Dashboard", className="ml-2"), width=3
                    ),
                    align="center",
                    no_gutters=True,
                ),
